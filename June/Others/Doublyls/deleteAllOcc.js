...
const findAllOcc = function (head,key) {
    let pos = [];
    let current = head;
    let index = 0;
    while (current !== null) {
...
const ls = makeLs([1,2,3,2,5,2,2]);
// console.log(deleteAtGivenPosition(ls,));
function deleteAllOcc(head,key){
    let pos = findAllOcc(head,key)
    let diff = 0;
    for (let i = 0; i < pos.length; i++) {
        head = deleteAtGivenPosition(head,pos[i] - diff);
        diff++;
...
console.log([1,2,3,2,5,2,2]);
console.log(printLs(ans));
